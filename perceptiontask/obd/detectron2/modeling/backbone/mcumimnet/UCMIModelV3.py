...
import torch
from .tinymim import tinymim_vit_tiny_patch16
from .vit import Block
from .multiAtten import TransformerAttenBlockVpaper as AttenBlock
import os
from torchvision.transforms.functional import normalize
from detectron2.modeling.backbone.backbone import Backbone
from .pos_embed import interpolate_pos_embed, interpolate_pos_encoding


class UCMIMNetV3(Backbone):
    def __init__(self,out_feature="last_feat"):
        super(UCMIMNetV3, self).__init__()
        self.encoder = tinymim_vit_tiny_patch16()
        pretrained_ckpt = '/home/lpw/fastssd/lpw/DeFusionv2/TinyMIM-PT-Tstar.pth'
        ...
        self.conv1x1 = nn.Sequential(nn.LayerNorm(decoder_embed_dim), nn.Linear(decoder_embed_dim, decoder_img_dim), nn.GELU())

        self.normalize_mean = [0.485, 0.456, 0.406]
        self.normalize_std = [0.229, 0.224, 0.225]

        self._out_feature_channels = {out_feature: decoder_img_dim}
        self._out_feature_strides = {out_feature: 16}
        self._out_features = [out_feature]

        self.init_pos_embed = True
    ...
    def init_pos(self) -> None:
        pos_embed = interpolate_pos_encoding(4096, 192, self.encoder, 16, (16, 16), 1024, 1024)
        # pos_embed = interpolate_pos_encoding(1936, 192, self.encoder, 16, (16, 16), 704, 704)
        self.encoder.pos_embed = nn.Parameter(pos_embed, requires_grad=False)
        # self.encoder.patch_embed.img_size = (704, 704)
        self.encoder.patch_embed.img_size = (1024, 1024)
        self.init_pos_embed = False

    def forward(self, img_dict):
        if self.init_pos_embed:
            self.init_pos()
        img1, img2 = img_dict['ir'].tensor, img_dict['vis'].tensor

        # print("img1 shape", img1.shape, img2.shape)

        img1 = normalize(img1, self.normalize_mean, self.normalize_std)
        img2 = normalize(img2, self.normalize_mean, self.normalize_std)
        # print("img1 img2", img1.max(), img2.max(), img1.min(), img2.min())
        ...
        # print("the inital residual 1", residual_fea1.max(), residual_fea1.min(), residual_fea1.shape)
        # print("the inital residual 2", residual_fea2.max(), residual_fea2.min(), residual_fea2.shape)

        enc_fea1 = enc_feas1[-1]
        enc_fea2 = enc_feas2[-1]

        # print("the inital feas 1", enc_fea1.max(), enc_fea1.min(), enc_fea1.shape)
        # print("the inital feas 2", enc_fea2.max(), enc_fea2.min(), enc_fea2.shape)

        com_img = self.decoder_common_blocks[0](enc_fea1, enc_fea2) + self.decoder_common_blocks[0](enc_fea2, enc_fea1)
        ...
        # print("the inital c&u:c", com_img.max(), com_img.min(), com_img.shape, com_img[0])
        # print("the inital c&u:u1", uni_img1.max(), uni_img1.min(), uni_img1.shape)
        # print("the inital c&u:u2", uni_img2.max(), uni_img2.min(), uni_img2.shape)
        # print("the residual inital c&u:c", residual_com_img.max(), residual_com_img.min(), residual_com_img.shape)
        ...
        # print("the inital values", fuse_img.max(), fuse_img.min(), fuse_img.shape)

        # return fuse_img
        fuse_img = self.conv1x1(fuse_img)
        fuse_img = self.unpatchifyc(fuse_img[:, 1:, :], c=768, p=1)
        # print("values", fuse_img.max(), fuse_img.min(), fuse_img.shape)

        # print("shape fused image", fuse_img.shape)

        outputs = {self._out_features[0]: fuse_img}
        return outputs
