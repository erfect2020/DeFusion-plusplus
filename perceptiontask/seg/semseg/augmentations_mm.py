...
class ResizeV2:
    ...
    def __call__(self, sample: list) -> list:
        ...
        # scale the image
        scale_factor = self.size[0] / max(H, W)
        nH, nW = round(H * scale_factor), round(W * scale_factor)
        nH = nW =  max(nH, nW)
        for k, v in sample.items():
            if k == 'mask':
        ...
        pad_top = (self.size[0] - nH) // 2
        pad_bottom = self.size[0] - nH - pad_top
        pad_left = (self.size[1] - nW) // 2
        pad_right = self.size[1] - nW - pad_left
        ...
        #     padding = [0, 0, self.size[1] - sample['img'].shape[2], self.size[0] - sample['img'].shape[1]]
        #     for k, v in sample.items():
        #         if k == 'mask':
        #             sample[k] = TF.pad(v, padding, fill=self.seg_fill)
        #         else:
