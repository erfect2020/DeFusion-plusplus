...
from semseg.models.cumaelayers.multiAtten import TransformerAttenBlockVpaper as AttenBlock
...
class UCMIMNetV3(nn.Module):
    def __init__(self, backbone: str = 'CMNeXt-B0', num_classes: int = 25, modals: list = ['img', 'depth', 'event', 'lidar'], fpn1_norm='BN', out_indices=[3, 5, 7, 11]):
        ...
        self.fuse_fpn1 = AttenBlock(decoder_embed_dim, decoder_num_heads, common=False, dim_feedforward=int(mlp_ratio * decoder_embed_dim))
        ...
        self.fuse_fpn2 = AttenBlock(decoder_embed_dim, decoder_num_heads, common=False,
        ...
        self.fuse_fpn3 = AttenBlock(decoder_embed_dim, decoder_num_heads, common=False,
                                    dim_feedforward=int(mlp_ratio * decoder_embed_dim))
    ...
    def init_pos(self) -> None:
        pos_embed = interpolate_pos_encoding(1600, 192, self.encoder, 16, (16, 16), 640, 640)
        self.encoder.pos_embed = nn.Parameter(pos_embed, requires_grad=False)
        self.encoder.patch_embed.img_size = (640, 640)
        self.init_pos_embed = False

    def auto_init_pos(self,img) -> None:
        imgsize = img.shape[2]
        pos_embed = interpolate_pos_encoding((imgsize/16)**2, 192, self.encoder, 16, (16, 16), imgsize, imgsize)
        self.encoder.pos_embed = nn.Parameter(pos_embed, requires_grad=False)
        self.encoder.patch_embed.img_size = (imgsize, imgsize)
        # self.init_pos_embed = False
    ...
    def forward(self, imgs):
        ...
        residual_fuse_img = self.decoder_fuse_skipconn(residual_com_img,
                                                       residual_uni_img1) + self.decoder_fuse_skipconn(
...
class TestGrids():
    ...
    def grids(self):
        ...
        step_i = step_j = 106 #310  #21 # 450 #53   24 80 113 22 80 119 21 84 124
