...
class EVA2(nn.Module):
    ...
    def __init__(
            self, backbone="EVA2", number_cls=9,dataset_cfg="",
            img_size=480,
            patch_size=16,
            in_chans=3,
            num_classes=9,
        ...
        self.decode_head = UPerHead(in_channels=[embed_dim * 1] * 4, channel=128, num_classes=9, scales=(1, 2, 3, 6))
        self.fpnup = nn.Sequential(
                nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=2, stride=2),
                nn.SyncBatchNorm(embed_dim),
                nn.GELU(),
                nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=2, stride=2),
    ...
    def forward(self, x):
        ...
        x[0] = self.fpnup(x[0])
        class_feature = self.decode_head(x)
        # print("clss f",class_feature.shape)
