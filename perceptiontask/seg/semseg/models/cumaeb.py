...
from semseg.models.cumaelayers.tinymim import tinymim_vit_tiny_patch16, tinymim_vit_base_patch16
...
class UCMIMNetVITB(nn.Module):
    def __init__(self, backbone: str = 'CMNeXt-B0', num_classes: int = 25, modals: list = ['img', 'depth', 'event', 'lidar'], fpn1_norm='BN', out_indices=[3, 5, 7, 11]):
        super(UCMIMNetVITB, self).__init__()
        self.encoder = tinymim_vit_base_patch16()
        ...
        self.conv1x1 = nn.Sequential(nn.LayerNorm(decoder_embed_dim), nn.Linear(decoder_embed_dim, 1280), nn.GELU())
        ...
        # self.decode_head = UPerHead(in_channels=[768 * 4, 768 * 4, 768 * 4, 768 * 4], in_index=[0, 1, 2, 3],
        #                             channels=192, out_channels=150, num_classes=150)
    ...
    def init_pretrained(self, pretrained: str = None) -> None:
        if pretrained:
            ...
            print(msg)
        # print("shape is", self.encoder.pos_embed.shape)
        pos_embed = interpolate_pos_encoding(900, 768, self.encoder, 16, (16, 16), 480, 480)
