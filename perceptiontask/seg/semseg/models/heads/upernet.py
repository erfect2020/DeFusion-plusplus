...
class UPerHead(nn.Module):
    ...
    def __init__(self, in_channels, channel=128, num_classes: int = 19, scales=(1, 2, 3, 6)):
        ...
        self.conv_seg = nn.Conv2d(channel, num_classes, kernel_size=1, padding=0, stride=1)
        # self.conv_segv1 = nn.Conv2d(channel, num_classes,  kernel_size=1, padding=0, stride=1)
    ...
    def forward(self, features: Tuple[Tensor, Tensor, Tensor, Tensor]) -> Tensor:
        ...
        for i in reversed(range(len(features)-1)):
            # print("i",i, "features[i]", features[i].shape)
            feature = self.fpn_in[i](features[i])
            # print("after fpn_in", feature.shape, f.shape)
            f = feature + F.interpolate(f, size=feature.shape[-2:], mode='bilinear', align_corners=False)
            # print("after fpn_out", self.fpn_out[i](f).shape)
            fpn_features.append(self.fpn_out[i](f))

        fpn_features.reverse()
        # print("fpn features", fpn_features[0].shape)
