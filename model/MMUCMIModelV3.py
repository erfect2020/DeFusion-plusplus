...
class MUCMIMNetV3(nn.Module):
    ...
    def forward(self, img1, img2, modality=None):
        ...
        if modality == 'irvis' or modality == 'ctmr':
            ...
            uni_img2 = self.decoder_unique_blocks[0](enc_fea1, enc_fea2) + self.decoder_unique_residual(enc_fea2)
            residual_uni_img2 = self.decoder_unique_skipconn(residual_fea1,
                                                             residual_fea2) + self.decoder_unique_residual_skipconn(
                residual_fea2)

            uni_img1 = self.decoder_unique_blocks[0](enc_fea2, enc_fea1) + self.decoder_unique_residual(enc_fea1)
            residual_uni_img1 = self.decoder_unique_skipconn(residual_fea2,
                                                             residual_fea1) + self.decoder_unique_residual_skipconn(
                residual_fea1)
            ...
            rec_img = self.decoder_fuse_blocks[0](com_mimg, uni_mimg1) + self.decoder_fuse_blocks[0](com_mimg,
                                                                                                     uni_mimg2)
            ...
            residual_rec_img = self.decoder_fuse_skipconn(residual_com_mimg,
                                                          residual_uni_mimg1) + self.decoder_fuse_skipconn(
